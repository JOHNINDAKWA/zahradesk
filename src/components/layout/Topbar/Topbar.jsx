  ...
  return (
    <div className="topbar">
      <div className="topbar-left">
        <FiMenu className="menu-icon desktop-menu" onClick={() => setCollapsed(!collapsed)} />

        <FiMenu
          className="menu-icon mobile-menu"
          onClick={() => {
      ...
      <div className="topbar-right">
        ...
        <Link to="/new-ticket" className="create-ticket-btn">
          + Create Ticket
        </Link>
        ...
        <Link to="/profile">
          <FiSettings className="topbar-icon" />
        </Link>
        ...
        <div className="profile-wrapper" ref={dropdownRef}>
          ...
          {dropdownOpen && (
            <div className="dropdown-menu2 theme-dropdown2">
              <div
                className="dropdown-item2 toggle-theme2"
                onClick={() => setDarkMode(!darkMode)}
              >
                {darkMode ? <FiSun /> : <FiMoon />}
                <span>{darkMode ? "Light Theme" : "Dark Theme"}</span>
              </div>
              <Link to="/profile" className="dropdown-item2">
                <FiSettings /> Profile Settings
              </Link>
              <div className="dropdown-item2">
                <FiUser /> Customer Portal
              </div>
              <div className="dropdown-item2">
                <FiHelpCircle /> Help
              </div>
              <Link to="/login" className="dropdown-item2">
                <FiLogOut /> Sign Out
              </Link>
            </div>
          )}
